...
class ConnectorControllerIntegrationTest {
    ...
    void aConnectorNeverSynchronizedIsHonestlyReported404NotFabricated() {
        // Aucun service de synchronisation MISP n'existe encore dans ce
        // lot : aucune ligne n'a jamais ete creee pour lui — le 404 est
        // le comportement honnete, pas une donnee inventee pour combler.
        String admin = loginToken("admin", "IntegrationTest123!");

        ResponseEntity<String> misp = exchange(HttpMethod.GET, CONNECTORS + "/MISP", admin, String.class);

        assertThat(misp.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
    }
