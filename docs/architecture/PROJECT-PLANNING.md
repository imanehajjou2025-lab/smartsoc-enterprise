...
| 5 — Actions réelles | 🔶 **en cours** (1/2 connecteurs, Wazuh backend complet) | — | [#105](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/105) [#107](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/107) [#109](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/109) | Contrôle d'agents Wazuh : redémarrage (backend + frontend) et active-response `firewall-drop`/blocage d'IP (backend) livrés. Identifiants dédiés `smartsoc-actuator` (portée minimale, confirmés en réel), garde-fous non négociables tenus et partagés entre les deux actions (cible confirmée par saisie exacte, motif obligatoire, plafond 3/h compté séparément par type d'action, aucun retry, audit nominatif systématique), 572 tests backend + 48 tests frontend verts. Vérifié de bout en bout au navigateur/API (mode simulation, jamais de vraie action déclenchée). Restent : bouton frontend du blocage d'IP, Shuffle — voir journal PFE, 2026-08-10 |
