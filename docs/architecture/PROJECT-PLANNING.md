...
| 3 — VirusTotal | ✅ **terminée** | — | [#93](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/93) [#95](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/95) | Connecteur à la demande (cache TTL 24h + `@CircuitBreaker`+`@RateLimiter`), verdict dérivé des compteurs réels par « pire cas gagne », 524 tests backend verts (+35) ; bug transactionnel réel corrigé (`noRollbackFor`) trouvé par test WireMock ; ACL vérifiée sur 2 échantillons réels (IP + domaine). Bouton « Vérifier la réputation » dans le tiroir IOC (rôles d'écriture) + carte Connecteurs corrigée (même défaut que MISP : figée sur « Phase 3 »), vérifiés au navigateur en Docker reconstruit — voir journal PFE, 2026-08-09 |
| 4 — OpenSearch (Hunting) | ✅ **terminée** | — | [#102](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/102) | Blocage documenté dès l'origine (schéma d'index réel indisponible) levé par 2 échantillons réels capturés avant tout code ; bandes de sévérité `rule.level` confirmées avec Imane ; 543 tests backend verts (+15) ; zéro changement du port/service/API/écran (critère de fin tenu par construction). Chasse `SEVERITY=CRITICAL` exécutée par Imane au navigateur contre le labo SOC réel : 136 correspondances réelles — voir journal PFE, 2026-08-10 |
| 5 — Actions réelles | ✅ **terminée** (2/2 connecteurs) | — | [#105](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/105) [#107](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/107) [#109](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/109) [#111](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/111) [#113](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/113) [#114](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/114) [#115](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/115) | Contrôle d'agents Wazuh (redémarrage + active-response `firewall-drop`) **et** déclenchement de workflow Shuffle, backend + frontend pour les deux connecteurs. Identifiants dédiés (`smartsoc-actuator`, clé API Shuffle globale), garde-fous non négociables partagés (cible confirmée par saisie exacte, motif obligatoire, plafond horaire compté séparément par type d'action/incident, aucun retry, audit nominatif systématique). Shuffle : schéma d'API confirmé en réel avant tout code (webhook de déclenchement, liste d'exécutions pour la réconciliation — `.../executions/{id}` répond 404 sur cette version), réconciliation en lecture à la demande (pas de callback entrant en V1), certificat TLS régénéré côté SOC et capturé, connecteur passé en mode **live** réel avec statut `CONNECTED` confirmé. 605 tests backend + 55 tests frontend verts. Vérifié de bout en bout au navigateur contre le backend Docker réel — y compris en mode live pour Shuffle (lecture seule), jamais de déclenchement réel effectué. Phase 5 achevée — voir journal PFE, 2026-08-10 et 2026-08-11 |
| 6 — Validation complète | 🔶 **en cours** (volet Fonctionnel achevé, 1/6 volets) | — | [#117](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/117) [#118](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/118) [#119](https://github.com/imanehajjou2025-lab/smartsoc-enterprise/pull/119) | Volet « Fonctionnel » : campagne manuelle scénarisée, 14 modules parcourus avec des données réelles contre le backend Docker reconstruit, vérifiés sur `:8080` et `:5173` (détail complet dans `docs/rapport-pfe/phase-6-validation-fonctionnelle.md`). 2 écarts réels trouvés et corrigés en conditions réelles : (1) crash du Journal d'audit sur les 3 actions d'audit de la phase 5, jamais répercutées côté frontend (PR #117) ; (2) `CapabilityProbe` (ADR-014 §6.5, documenté depuis l'origine mais jamais construit) implémenté pour les **5/5 connecteurs** — Wazuh `4.12.0`, MISP `2.5.44`, Shuffle `onprem/docker`, VirusTotal (pas de version applicable, service cloud), OpenSearch `7.10.2` (PR #118, #119 — signalé par Imane en relisant la carte Connecteurs ; OpenSearch nécessitait un ajout de rôle `cluster:monitor/main` côté SOC, fait par Imane le 2026-08-11). Volets restants : Intégration des connecteurs, Résilience, Charge, Sécurité, Pré-production. |
